...
if ~isempty(kv.M)
    ...
    kv.spacing = (freqtoerb(kv.fmax) - freqtoerb(kv.fmin))/kv.M;
end
...
if fmin > 0
    g{1} = erblowpassfilter(filterfunc,g,a,fmin,fs,winbw,scal(1),M2,bwtruncmul,kv,flags);
end

if fmax < fs/2
    g{M2} = erbhighpassfilter(filterfunc,g,a,fmax,fs,winbw,scal(M2),M2,bwtruncmul,kv,flags);
end

function glow = erblowpassfilter(filterfunc,g,a,fmin,fs,winbw,scal,M2,bwtruncmul,kv,flags)
next_fc = max(erbtofreq(freqtoerb(fmin)-kv.spacing),0);
temp_fc = [];
...
while next_fc >= erbtofreq(-4)
    ...
    function ghigh = erbhighpassfilter(filterfunc,g,a,fmax,fs,winbw,scal,M2,bwtruncmul,kv,flags)
    next_fc = min(erbtofreq(freqtoerb(fmax)+kv.spacing),fs/2);
    temp_fc = [];
