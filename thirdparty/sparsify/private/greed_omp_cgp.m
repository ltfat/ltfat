...
% conjugate update direction update (Conjugate Gradient Pursuit) [1]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Usage
% [s, err_mse, iter_time]=greed_omp_cgp(x,P,m,'option_name','option_value')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Input
%   Mandatory:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Description
%   greed_omp_cgp performs a greedy signal decomposition. 
...
%   The orthogonal projection is calculated using a single conjugate direction
%   update in each iteration. See [1] for details.
%   
% References
%   [1] T. Blumensath and M.E. Davies, "Gradient Pursuits", submitted, 2007
% See Also
%   greed_qr, greed_omp_chol, greed_omp_cg, greed_omp_pinv, 
...
if strcmp(STOPCRIT,'M') 
    maxM=STOPTOL+length(find(s_initial));
else
    maxM=MAXITER+length(find(s_initial));
end
...
if initial_given ==1;
    ...
    if ~isempty(in)
        ...
        Residual    = x;
        DR          = Pt(Residual);
        for k=1:length(in) %Go backwards so that we can take of elements in NI from the rear.
             IN=[IN in(k)];
             if k==1
                 d(IN)=1;
                 PG(1)=1;
