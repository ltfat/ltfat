...
{ 
  ...
  LTFAT_COMPLEX *gwork = ltfat_malloc(L*sizeof(LTFAT_COMPLEX));
...
{ 
   ...
   plan.rect_plan_array = ltfat_malloc(lt2*sizeof(LTFAT_NAME(dgt_long_plan)));

   for (int win=0;win<lt2;win++)
   {
      plan.rect_plan_array[win]=LTFAT_NAME(dgt_long_init)(plan.f,plan.mwin+L*win,L,W,a*lt2,M,
							 plan.c_scratch,flags);
   }

   plan.mod = ltfat_malloc(N*sizeof(LTFAT_COMPLEX)); 
