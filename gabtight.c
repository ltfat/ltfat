#include "config.h"
#include "ltfat.h"

LTFAT_EXTERN void
LTFAT_NAME(gabtight_long)(const LTFAT_COMPLEX *g,
			  const int L, const int R, const int a,
			  const int M, LTFAT_COMPLEX *gd)
{
   LTFAT_COMPLEX *gf = ltfat_malloc(L*R*sizeof(LTFAT_COMPLEX));
   LTFAT_COMPLEX *gdf = ltfat_malloc(L*R*sizeof(LTFAT_COMPLEX));
  
   LTFAT_NAME(wfac)(g, L, R, a, M, gf);
   LTFAT_NAME(gabtight_fac)((const LTFAT_COMPLEX *)gf,L,R,a,M,gdf);
   LTFAT_NAME(iwfac)((const LTFAT_COMPLEX *)gdf,L,R,a,M,gd);

   ltfat_free(gdf);
   ltfat_free(gf);

}

LTFAT_EXTERN void
LTFAT_NAME(gabtightreal_long)(const LTFAT_REAL *g,
			      const int L, const int R, const int a,
			      const int M, LTFAT_REAL *gd)
{

   const int wfs = L; /* wfacreal_size(L,a,M); */

   LTFAT_COMPLEX *gf = ltfat_malloc(wfs*R*sizeof(LTFAT_COMPLEX));
   LTFAT_COMPLEX *gdf = ltfat_malloc(wfs*R*sizeof(LTFAT_COMPLEX));
  
   LTFAT_NAME(wfacreal)(g, L, R, a, M, gf);
   LTFAT_NAME(gabtightreal_fac)((const LTFAT_COMPLEX *)gf,L,R,a,M,gdf);
   LTFAT_NAME(iwfacreal)((const LTFAT_COMPLEX *)gdf,L,R,a,M,gd);

   ltfat_free(gdf);
   ltfat_free(gf);

}
