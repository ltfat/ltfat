...
% 1) Download and unpack 64bit binaries of the FFTW library from http://www.fftw.org/install/windows.html. Copy all *.dll and *.def files to the ltfat/mex directory.
...
if flags.do_clean
    ...
    [status,result]=system([make_exe, ' -f Makefile_mingw64 clean',' MEXEXT=',mexext]);
    cd([bp,'src']);
    [status,result]=system([make_exe, ' -f Makefile_mingw64 clean']);
    cd([bp,'mex']);
    [status,result]=system([make_exe, ' -f Makefile_mingw64 clean',...
                    ' MEXEXT=',mexext]);
...
disp('========= Compiling GPC ====================================');
cd([bp,'thirdparty',filesep,'PolygonClip']);
[status,result]=system([make_exe, ' -f Makefile_mingw64',...
                  ' MATLABROOT=','"',matlabroot,'"',...
                  ' MEXEXT=',mexext]);
%system('gcc -I../GPC PolygonClip.c ../GPC/gpc.c');
if(~status)
   disp('Done.');
else
...
disp('========= Checking for FFTW, creating include libs =========');
cd(bp);
...
if(~status)
   ...
   error('Failed to build LTFAT dll:\n %s',result);
end
...
if(~status)
   ...
   error('Failed to build MEX interfaces:\n %s',result);
end
