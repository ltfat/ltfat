...
void LTFAT_NAME(ltfatMexFnc)( int nlhs, mxArray *plhs[],int nrhs, const mxArray *prhs[] )
{
  ...
  LTFAT_NAME(gabtight_long)((const LTFAT_REAL (*)[2])g_combined, L, R, a, M, (LTFAT_REAL (*)[2])gd_combined);
  #else
  LTFAT_NAME(gabtightreal_long)(g_combined, L, R, a, M, gd_combined);
  #endif
...

{
   ...
   if (mxIsComplex(prhs[0]))
   {
      ...
      LTFAT_NAME_DOUBLE(gabtight_long)((const ltfat_complex*)g_combined,L,R,a,M,gd_combined);
   ...
   else
   {
      ...
      LTFAT_NAME_DOUBLE(gabtightreal_long)((const double*)mxGetPr(prhs[0]),
			L,R,a,M,
			mxGetPr(plhs[0]));
